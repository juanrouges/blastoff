...
const sample = nums.reduce(function (total, nextValue) {
...
console.log(extractValue(mentors, 'name'));
...
function vowelCount(strng) {
  let value = strng.toLowerCase().split('');
  const vowels = ['a', 'e', 'i', 'o', 'u'];
  return value.reduce(function (accumulator, nextValue) {
    if (!accumulator[nextValue] && vowels.includes(nextValue)) {
      accumulator[nextValue] = 1;
    } else {
      vowels.includes(nextValue) && (accumulator[nextValue] += 1);
    }
    return accumulator;
  }, {});
}
...
// specific value is false, the value should be placed in the
// second subarray.
